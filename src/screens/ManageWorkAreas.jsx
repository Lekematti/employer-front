...
function WorkAreaMap() {
    ...
    return (
        <div className="work-area-container">
            <MapContainer center={mapCenter} zoom={20} style={{ height: '100vh', width: 'calc(100% - 300px)' }}>
                <TileLayer
                ...
                {workAreas.map(workArea => (
                    <React.Fragment key={workArea.id}>
                        <Marker position={[workArea.latitude, workArea.longitude]}>
                            <Popup>{workArea.name}<br />{workArea.description}</Popup>
                        </Marker>
                        <Circle
            ...
            <div className="work-area-list">
                ...
                <ul>
    {workAreas.map(workArea => (
        <li key={workArea.id} onClick={() => handleWorkAreaClick(workArea.id, workArea.latitude, workArea.longitude)}>
            ...
            {userDetails[workArea.id] && (
               ...
               {userDetails[workArea.id].map(user => (
                   <li key={user.id}>
                       <img src={`http://localhost:3000/uploads/${user.picture}`} alt={user.name} />
                       <span className="user-name">{user.name}</span>
                       {user.workLog ? (
                           <span className="time-box">
                               {new Date(user.workLog.start_time).toLocaleTimeString()}
                           </span>
                       ) : (
