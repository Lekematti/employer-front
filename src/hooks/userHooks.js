...
const userHooks = () => {
  ...
  const getUsersByWorkAreaId = async (workAreaId) => {
    try {
      ...
      const res = await axios.get(
        API_URL + "user/" + "workarea/" + workAreaId,
        options
      );
      return res.data;
    } catch (err) {
        ...
        users.map(async (user) => {
          try {
            const logRes = await axios.get(
              API_URL + `worklogs/${user.id}/${workAreaId}/today`,
              options
            );
  ...
  const getUserDetailsByWorkAreaId = async (workAreaId) => {
    ...
    try {
      const response = await axios.get(
        `${API_URL}user/workArea/${workAreaId}/users`,
        options
      );
  ...
  const getAllUsersByCompanyId = async (companyId) => {
    try {
      ...
      const res = await axios.get(API_URL + "user/company/" + companyId, options);
  ...
  return {
    getUserById,
    getUsersByWorkAreaId,
    getUsersAndLogsByWorkAreaId,
    getUserDetailsByWorkAreaId,
    getEmployeeById,
